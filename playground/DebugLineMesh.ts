...
class DynLineMesh extends THREE.LineSegments {
  ...
  add(
    p0:
      | Float32Array
      | THREE.Mesh<THREE.BufferGeometry, THREE.Material | THREE.MeshPhongMaterial>
      | number[],
    ...
    if (isDash) {
      const len = Math.sqrt((p1[0] - p0[0]) ** 2 + (p1[1] - p0[1]) ** 2 + (p1[2] - p0[2]) ** 2);
...
//#region SUPPORT
function _newDynLineMeshGeometry(
  aVerts: number | Iterable<number> | ArrayBuffer | ArrayLike<number>,
  aColor: number | Iterable<number> | ArrayBuffer | ArrayLike<number>,
  aConfig: number | Iterable<number> | ArrayBuffer | ArrayLike<number>,
  doCompute = true
) {
...
function glColor(hex: number, out: number[] | null = null) {
