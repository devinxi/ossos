...
class BoneViewMesh extends THREE.Mesh {
  constructor(arm: Armature, color = "white", useDepthTest = true) {
  ...
  updateFromPose(pose: Pose) {
    let material = this.material as THREE.RawShaderMaterial;
    const rot = material.uniforms.boneRot.value;
    const pos = material.uniforms.bonePos.value;
    const scl = material.uniforms.boneScl.value;
...
function instanceData(arm: Armature) {
  const bCnt = arm.bones.length;
