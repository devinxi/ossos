...
class ShapePointsMesh extends THREE.Points {
  ...
  add(
    pos: number[] | THREE.Mesh<THREE.BufferGeometry, THREE.MeshPhongMaterial>,
  ...
  setColorAt(idx: number, color: any) {
  ...
  getPosAt(idx: number) {
    idx *= 3;
    return [this._verts[idx + 0], this._verts[idx + 1], this._verts[idx + 2]];
  }
  ...
  _updateGeometry() {
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if (
      this._verts.length > bVerts.array.length ||
...
function _newShapePointsMeshGeometry(
  aVerts: number | Iterable<number> | ArrayLike<number> | ArrayBuffer,
  aColor: number | Iterable<number> | ArrayLike<number> | ArrayBuffer,
  aConfig: number | Iterable<number> | ArrayLike<number> | ArrayBuffer,
  doCompute = true
) {
...
function glColor(hex: number, out = null) {
